...
DECLARE @start_time DATETIME, @end_time DATETIME;
	BEGIN TRY
		...
		PRINT '-------------------------------------------';
	-- Loading the clean tables to the silver layer
	...
	FROM(
		SELECT *,
		ROW_NUMBER() OVER(PARTITION BY cst_id ORDER BY cst_create_date DESC) AS last_info
		FROM bronze.crm_cust_info
		WHERE cst_id IS NOT NULL) T WHERE last_info = 1;
	...
	SELECT
	prd_id,
	REPLACE(SUBSTRING(prd_key,1, 5), '-', '_') AS cat_id, -- created to join with erp_px_cat_g1v2 table
	SUBSTRING(prd_key, 7, LEN(prd_key)) AS prd_key, -- created to join with crm_sales_details table
	prd_nm,
	...
	END AS sls_due_dt,
	-- when sls_sales is null or negative we use a formula to calculate it 
	...
	END AS sls_sales,
	sls_quantity,
	-- when sls_price is null or negative we use a formula to calculate it 
	...
	PRINT '>>>> loading Data to: silver.erp_cust_az_12';
	...
	BEGIN CATCH
		PRINT '==========================================='
		PRINT 'Error !! Loading silver layer failed'
